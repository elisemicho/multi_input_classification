...
class MultiInputCNN(object):
    ...
    def _build_model(self):
        ...
        char_embedding = Dropout(self.model_config.emb_dropout_p)(char_embedding)
        ...
        phone_CZ_embedding = Dropout(self.model_config.emb_dropout_p)(phone_CZ_embedding)
        ...
        phone_EN_embedding = Dropout(self.model_config.emb_dropout_p)(phone_EN_embedding)
        ...
        phone_HU_embedding = Dropout(self.model_config.emb_dropout_p)(phone_HU_embedding)
        ...
        phone_RU_embedding = Dropout(self.model_config.emb_dropout_p)(phone_RU_embedding)

        embed_input = Input(shape=(self.input_acoustic_size,), dtype='float32', name='embed_input')

        # Convolutional layers on chars and phones
        conv_pools = []
        for embedding in [char_embedding, phone_CZ_embedding, phone_EN_embedding, phone_HU_embedding, phone_RU_embedding]:
            for filter_size in self.model_config.filter_sizes:

                # Concatenate with acoustic embeddings
                l_concat = Concatenate()([embedding,embed_input])
                l_zero = ZeroPadding1D((filter_size-1,filter_size-1))(l_concat)
                l_conv = Conv1D(filters=self.model_config.num_filters, kernel_size=filter_size, padding='same', activation='tanh')(l_zero)
                l_drop = Dropout(self.model_config.conv_dropout_p)(l_conv)
        ...
        for fl in self.model_config.fully_connected_layers:
            ...
            x = Dropout(self.model_config.dense_dropout_p)(x)
